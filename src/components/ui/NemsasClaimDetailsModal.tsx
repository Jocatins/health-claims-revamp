...
const NemsasClaimDetailsModal: React.FC<NemsasClaimDetailsModalProps> = ({ open, onClose, claimId }) => {
  const [loading, setLoading] = useState(false);
  const [updating, setUpdating] = useState(false);
  const [error, setError] = useState<string>('');
  const [claim, setClaim] = useState<EditableClaim | null>(null);
  const [originalClaim, setOriginalClaim] = useState<EditableClaim | null>(null);
  ...
  const loadClaim = useCallback(async () => {
    ...
    try {
      ...
      if (res?.isSuccess && res?.data) {
        const data = res.data;
        const mappedItems = Array.isArray(data.claimItems) ? data.claimItems.map(normalizeItem) : [];
  ...
  const handleItemChange = (idx: number, field: keyof EditableClaimItem, value: string | number) => {
    if (!claim) return;
    const updated = { ...claim };
  ...
  const isValid = () => {
    ...
    const required = [claim.claimName, claim.patientName, claim.patientNumber, claim.phoneNumber, claim.serviceType];
    if (required.some(v => !v || !v.toString().trim())) return false;
    if (!claim.claimItems.length) return false;
    if (claim.claimItems.some(ci => !ci.name.trim() || ci.amount <= 0)) return false;
    return true;
  };
    ...
    try {
      const payload: UpdateNemsasClaimRequest = {
        ...
        claimItems: claim.claimItems.map(ci => ({
          ...
          name: ci.name,
          amount: ci.amount,
          claimStatus: ci.originalStatusCode !== undefined
            ? textToStatusCode[String(ci.claimStatus)] ?? ci.originalStatusCode
  ...
  return (
    <Modal open={open} onClose={onClose} title="NEMSAS Claim Details" width="760px">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxHeight: '70vh', overflowY: 'auto' }}>
          ...
          {editing && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <Button variant="outline" onClick={addItem} className="self-start">+ Add Item</Button>
              <div style={{ display: 'flex', gap: 12 }}>
                <Button onClick={handleUpdate} disabled={!isValid() || updating}>{updating ? 'Updating...' : 'Save Changes'}</Button>
                <Button variant="outline" onClick={() => { if (originalClaim) setClaim(originalClaim); setEditing(false); }}>Cancel</Button>
              </div>
            </div>
          )}
        </div>

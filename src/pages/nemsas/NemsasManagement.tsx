...
import { exportClaimsReport as exportClaimsReportApi, fetchClaimDetails } from '../../services/api/claimsApi';
...
export const NemsasManagement = () => {
  ...
  const { claims: reduxClaims, loading, error } = useAppSelector((state) => state.claims);
    ...
    <div style={{ padding: '32px' }}>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={
            <div>
              ...
              <p><strong>ProviderId:</strong> {currentUser.providerId}</p>
              <p><strong>HmoId:</strong> {currentUser.hmoId}</p>
              <p style={{ marginTop: '8px' }}>This means no claims exist in the database for these IDs.</p>
            </div>
          }
          action={
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          <Table
            headers={["S/N", "Enrollee name", "Enrollee Id", "Submitted date", "Total amount", "Status", "Action"]}
            rows={tableClaims.map((claim, index) => [
              ...
              <span 
                key={`status-${claim.id}`}
                style={{ 
                  color: statusColor[claim.status as keyof typeof statusColor] || '#000', 
                  fontWeight: 600 
                }}
              >
                {claim.status}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16, fontSize: 14, color: '#6b6f80' }}>
            <span>Showing 1-{tableClaims.length} of {tableClaims.length}</span>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Button variant="outline" onClick={() => { setShowNemsasClaimModal(true); setShowCreateModal(false); }}>Single claim</Button>
